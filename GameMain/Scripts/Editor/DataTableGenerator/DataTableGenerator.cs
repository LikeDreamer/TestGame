...
namespace AltarOfSword.Editor.DataTableTools
{
    public sealed class DataTableGenerator
    {
        ...
        private static bool OnOpenAsset(int instanceID, int line)
        {
            ...
            if (assetObj == null) return true;
            string assetPath = AssetDatabase.GetAssetPath(assetObj);
            string directoryName = Path.GetDirectoryName(assetPath).Replace("\\", "/");
            string extension = Path.GetExtension(assetPath);
            if (directoryName == DataTablePath && extension == ".txt")
            {
                assetPath = Path.Combine(Path.GetDirectoryName(Application.dataPath), assetPath);
                assetPath = assetPath.Replace("/", "\\");
                System.Diagnostics.Process.Start("excel.exe", assetPath);
                return true;
            }
            return false;
        }
        ...
        private static void DataTableCodeGenerator(DataTableProcessor dataTableProcessor, StringBuilder codeContent, object userData)
        {
            ...
            codeContent.Replace("__DATA_TABLE_NAME_SPACE__", "AltarOfSword");
